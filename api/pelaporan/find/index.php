<?php
    ...
    if (!(isset($_GET["id_pelaporan"]) && $_GET["id_pelaporan"])) {
        $response = [
            "status_code" => 400,
            "data" => null,
    ...
    $query = "
        SELECT m.id, b.id, da.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, 
            ...
            plda.id, plkec.id, plkab.id, plp.id, pln.id, pln.nama, n.bendera, pln.status_aktif, 
            ...
            jp.id, jp.nama, jp.icon, jp.status_darurat, jp.status_aktif, pl.judul, pl.foto, 
            pl.keterangan, pl.waktu, pl.latitude, pl.longitude
        FROM pelaporan pl, masyarakat m, banjar b, desa_adat da, kecamatan kec, 
    ...
    $stmt->bind_result($masyarakat_id, $banjar_id, $desa_adat_id, $kecamatan_id,
        ...
        $title, $photo, $description, $time, $latitude, $longitude);
    $stmt->fetch();
    $stmt->close();
    ...
    $petugas_reports = [];
    $query = "
        SELECT ptpl.id, pt.id, ip.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, p.nama, p.status_aktif, kab.nama, kab.status_aktif, kec.nama, kec.status_aktif, jip.id, jip.nama, jip.status_aktif, ip.nama, ip.status_pelaporan, ip.status_aktif, pt.nama, pt.email, pt.nik, pt.avatar, pt.no_telp, pt.tanggal_lahir, pt.jenis_kelamin, pt.status_aktif, ptpl.foto, ptpl.status
        FROM petugas_pelaporan ptpl, petugas pt, instansi_petugas ip, jenis_instansi_petugas jip, kecamatan kec, kabupaten kab, provinsi p, negara n
        WHERE ptpl.id_petugas = pt.id
    ...
    $response = [
        ...
        "message" => "Data pelaporan berhasil diperoleh"
    ];
    echo json_encode($response);