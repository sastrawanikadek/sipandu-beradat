<?php
    ...
    $query = "
        SELECT a.id, da.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, p.nama, 
            ...
            nt.status_aktif, t.email, t.avatar, t.no_telp, t.tanggal_lahir, t.jenis_identitas, 
            t.no_identitas, t.jenis_kelamin, t.status_aktif, t.status_valid
    ...
    $stmt->bind_result($akomodasi_id, $desa_adat_id, $kecamatan_id, 
        ...
        $negara_tamu_id, $negara_tamu_name, $negara_tamu_flag, $negara_tamu_active_status, $email, $avatar, $phone, $date_of_birth, $identity_type, $identity_number, 
        $gender, $active_status, $valid_status);
    $stmt->fetch();
    $stmt->close();
    
    $query = "UPDATE tamu t SET t.nama = ? WHERE t.id = ?";
    $stmt = $mysqli->prepare($query);
