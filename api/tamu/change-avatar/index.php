<?php
    ...
    $query = "
        SELECT a.id, da.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, p.nama, 
            ...
            nt.status_aktif, t.nama, t.email, t.no_telp, t.tanggal_lahir, t.jenis_identitas, 
    ...
    $stmt->bind_result($akomodasi_id, $desa_adat_id, $kecamatan_id, 
        ...
        $name, $email, $phone, $date_of_birth, $identity_type, $identity_number, 
        $gender, $active_status, $valid_status);
    $stmt->fetch();
    $stmt->close();
    
    $avatar = upload_image("avatar");
    $query = "UPDATE tamu t SET t.avatar = ? WHERE t.id = ?";
    $stmt = $mysqli->prepare($query);
