<?php
    ...
    $query = "
        SELECT a.id, da.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, p.nama, 
            ...
            nt.status_aktif, t.nama, t.email, t.username, t.avatar, t.no_telp, t.tanggal_lahir, 
    ...
    $stmt->bind_result($akomodasi_id, $desa_adat_id, $kecamatan_id, 
        ...
        $name, $email, $username, $avatar, $phone, $date_of_birth, $identity_type, $identity_number, 
    ...
    $query = "
        SELECT rlt.id, rlt.latitude, rlt.longitude, rlt.waktu
        FROM riwayat_lokasi_tamu rlt
        WHERE rlt.id_tamu = ?
    ";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("s", $id);
    ...
    $stmt->bind_result($history_id, $history_latitude, $history_longitude, $history_time);
    
    while ($stmt->fetch()) {
        array_push($histories, [
            "id" => $history_id,
            "latitude" => $history_latitude,
            "longitude" => $history_longitude,
            "time" => $history_time
        ]);
    }
    ...
    $response = [
        ...
        "message" => "Data riwayat lokasi tamu berhasil diperoleh"
    ];
