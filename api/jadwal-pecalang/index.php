<?php
    ...
    $query = "
        SELECT pda.id, m.id, b.id, da.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, 
            ...
            m.nama, m.avatar, m.no_telp, m.tanggal_lahir, m.nik, m.jenis_kelamin, m.kategori, 
            m.status_valid, pda.otoritas_sirine, pda.status_aktif
    ...
    $stmt->store_result();
    $stmt->bind_result($pecalang_id, $masyarakat_id, $banjar_id, 
        $desa_adat_id, $kecamatan_id, $kabupaten_id, $provinsi_id, 
        ...
        $active_status_active_status, $name, $avatar, $phone, $date_of_birth, $nik, $gender, $category, $valid_status,
        $sirine_authority, $pecalang_active_status);
    ...
    while($stmt->fetch()) {
        ...
        $query = "
            SELECT jpcl.tanggal_mulai, jpcl.tanggal_selesai 
            FROM jadwal_pecalang jpcl 
            WHERE jpcl.id_pecalang = ?
        ";
        $stmt2 = $mysqli->prepare($query);
        $stmt2->bind_param("s", $pecalang_id);
        ...
        $stmt2->bind_result($start_date, $end_date);
        
        while ($stmt2->fetch()) {
            if ($start_date == $end_date) {
                if (!in_array($start_date, $days)) {
                    array_push($days, $start_date);
                }
            } else {
                $start_date_datetime = new DateTime($start_date);
                $end_date_datetime = new DateTime($end_date);
                $interval = $start_date_datetime->diff($end_date_datetime, true);
                
                if (!in_array($start_date_datetime->format("Y-m-d"), $days)) {
                    array_push($days, $start_date_datetime->format("Y-m-d"));
                }
                
                for ($i = 0; $i < intval($interval->format("%a")); $i++) {
                    $start_date_datetime->add(new DateInterval('P1D'));
    ...
    $response = [
        "status_code" => 200,
        "data" => $data,
        "message" => "Data jadwal pecalang desa adat berhasil diperoleh"
    ];
