<?php
    ...
    $query = "
        SELECT rlm.id, rlm.latitude, rlm.longitude, rlm.waktu
        FROM riwayat_lokasi_masyarakat rlm
        WHERE rlm.id_masyarakat = ?
    ";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("s", $id);
    ...
    $response = [
        ...
        "message" => "Data riwayat lokasi masyarakat berhasil diperoleh"
    ];
    echo json_encode($response);