<?php
    ...
    $mysqli = connect_db();
    $query = "
        SELECT aa.id, aa.password 
        FROM admin_akomodasi aa
        WHERE aa.status_aktif = 1
        AND aa.username = ?
    ";
    ...
    $tokens = generate_jwt_token($user_id, "admin_akomodasi");
    ...
    $query = "INSERT INTO access_token_admin_akomodasi VALUES (NULL, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
    ...
    $query = "INSERT INTO refresh_token_admin_akomodasi VALUES (NULL, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
