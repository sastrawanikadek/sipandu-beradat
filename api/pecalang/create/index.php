<?php
    ...
    if (!(isset($_POST["id_masyarakat"]) && $_POST["id_masyarakat"] &&
        isset($_POST["sirine_authority"]) && isset($_POST["prajuru_status"]))) {
        $response = [
            "status_code" => 400,
            "data" => null,
    ...
    $query = "SELECT COUNT(*) FROM pecalang pda WHERE pda.id_masyarakat = ?";
    $stmt = $mysqli->prepare($query);
    ...
    if ($total > 0) {
        $response = [
            "status_code" => 400,
            "data" => null,
            "message" => "Data pecalang sudah ada"
        ];
        echo json_encode($response);
        exit();
    }
    ...
    $query = "INSERT INTO pecalang VALUES (NULL, ?, 0, 0, ?, ?, 1)";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("sss", $id_masyarakat, $sirine_authority, $prajuru_status);
