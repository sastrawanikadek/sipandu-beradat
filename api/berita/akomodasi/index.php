<?php
    ...
    $query = "
        SELECT ba.id, aa.id, pa.id, a.id, da.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, 
            ...
            aa.status_super_admin, ba.judul, ba.foto, ba.keterangan, ba.waktu, 
            ba.status_aktif
        FROM berita_akomodasi ba, admin_akomodasi aa, pegawai_akomodasi pa, akomodasi a, desa_adat da, kecamatan kec, kabupaten kab, provinsi p, negara n
        WHERE ba.id_admin_akomodasi = aa.id
        AND aa.id_pegawai = pa.id
    ...
    if (isset($_GET["id_admin_akomodasi"])) {
        $query .= " AND ba.id_admin_akomodasi = ?";
        array_push($params, $_GET["id_admin_akomodasi"]);
    }
    
    if (isset($_GET["active_status"])) {
        $active_status_param = json_decode($_GET["active_status"]);
        $query .= " AND ba.status_aktif = ?";
    ...
    $stmt->bind_result($id, $admin_id, $pegawai_id, $akomodasi_id, $desa_adat_id, $kecamatan_id, $kabupaten_id, $provinsi_id, 
        ...
        $pegawai_date_of_birth, $pegawai_nik, $pegawai_gender, $pegawai_active_status, $email, $active_status, $super_admin_status, $title, $cover, $content, $time, $news_active_status);
    ...
    $response = [
        "status_code" => 200,
        "data" => $data,
        "message" => "Data berita akomodasi berhasil diperoleh"
    ];
