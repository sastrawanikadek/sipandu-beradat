<?php
    ...
    $news_active_status = intval(json_decode($_POST["active_status"]));
    ...
    $query = "SELECT foto FROM $table_name WHERE id = ? AND id_$role = ?";
    $stmt = $mysqli->prepare($query);
    ...
    ";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("sssssss", $title, $cover, $content, $time, $news_active_status, $news_id, $id);
