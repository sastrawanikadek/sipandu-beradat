<?php
    ...
    $query = "
        SELECT bda.id, ad.id, m.id, b.id, da.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, p.nama, p.status_aktif, kab.nama, kab.status_aktif, kec.nama, kec.status_aktif, da.nama, da.latitude, da.longitude, da.status_aktif, b.nama, b.status_aktif, sam.id, sam.nama, sam.status, sam.status_aktif, m.nama, m.email, m.avatar, m.no_telp, m.tanggal_lahir, m.nik, m.jenis_kelamin, m.kategori, m.status_valid, ad.status_aktif, ad.status_super_admin, bda.judul, bda.foto, bda.keterangan, bda.waktu, bda.status_aktif
        FROM berita_desa_adat bda, admin_desa ad, masyarakat m, banjar b, desa_adat da, kecamatan kec, kabupaten kab, provinsi p, negara n, status_aktif_masyarakat sam
        WHERE bda.id_admin_desa = ad.id
    ...
    if (isset($_GET["id_desa_adat"])) {
        $query .= " AND b.id_desa = ?";
        array_push($params, $_GET["id_desa_adat"]);
    }
    
    if (isset($_GET["id_admin_desa"])) {
        $query .= " AND bda.id_admin_desa = ?";
        array_push($params, $_GET["id_admin_desa"]);
    }
    
    if (isset($_GET["active_status"])) {
        $active_status_param = json_decode($_GET["active_status"]);
        $query .= " AND bda.status_aktif = ?";
    ...
    $stmt->store_result();
    $stmt->bind_result($id, $admin_id, $masyarakat_id, $banjar_id, $desa_adat_id, $kecamatan_id, $kabupaten_id, $provinsi_id, $negara_id, $negara_name, $negara_flag, $negara_active_status, $provinsi_name, $provinsi_active_status, 
        ...
        $super_admin_status, $title, $cover, $content, $time, $news_active_status);
    ...
    while($stmt->fetch()) {
        ...
        array_push($data, [
            ...
            "content" => $content,
            "time" => $time,
            "active_status" => boolval($news_active_status)
        ]);
    }
    ...
    $response = [
        "status_code" => 200,
        "data" => $data,
        "message" => "Data berita desa adat berhasil diperoleh"
    ];
