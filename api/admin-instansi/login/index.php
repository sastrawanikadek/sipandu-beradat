<?php
    ...
    $mysqli = connect_db();
    $query = "
        SELECT ap.id, pt.password 
        FROM admin_petugas ap, petugas pt
        WHERE ap.id_petugas = pt.id
        AND ap.status_aktif = 1
        AND pt.username = ?
    ";
    ...
    $tokens = generate_jwt_token($user_id, "admin_petugas");
    ...
    $query = "INSERT INTO access_token_admin_petugas VALUES (NULL, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
    ...
    $query = "INSERT INTO refresh_token_admin_petugas VALUES (NULL, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
