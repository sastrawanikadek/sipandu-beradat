<?php
    ...
    $query = "
        SELECT kt.id, kt.status, IF(kt.id_tamu=?, 1, 0)
        FROM kerabat_tamu kt
        WHERE (kt.id_tamu = ? AND kt.id_kerabat = ?)
        OR (kt.id_tamu = ? AND kt.id_kerabat = ?)
    ";
    $stmt = $mysqli->prepare($query);
