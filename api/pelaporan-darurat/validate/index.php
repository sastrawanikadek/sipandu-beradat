<?php
    ...
    $query = "
        UPDATE pecalang_pelaporan_darurat ppdr
        SET ppdr.foto = ?, 
            ppdr.status = ?
        WHERE ppdr.id_pecalang = ?
        AND ppdr.id_pelaporan_darurat = ?
    ";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ssss", $photo, $valid_status, $id_pecalang, $id_pelaporan_darurat);
    ...
    if ($status && intval($status) == -1) {
        $query = "
            SELECT COUNT(*) 
            FROM laporan_darurat_tidak_valid ldrtv 
            WHERE ldrtv.id_pelaporan_darurat = ?
        ";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param("s", $id_pelaporan_darurat);
        ...
        if ($total == 0) {
            $query = "INSERT INTO laporan_darurat_tidak_valid VALUES (NULL, ?)";
            $stmt = $mysqli->prepare($query);
            $stmt->bind_param("s", $id_pelaporan_darurat);
        ...
        $query = "
            DELETE FROM laporan_darurat_tidak_valid 
            WHERE id_pelaporan_darurat = ?
        ";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param("s", $id_pelaporan_darurat);
        ...
        send_notifications($factory, $petugas_fcm_tokens, [
            ...
            "notification_data" => json_encode(["id" => $id_pelaporan_darurat])
        ]);
    }
