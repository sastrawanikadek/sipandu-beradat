<?php
    ...
    if (!(isset($_GET["id_pelaporan_tamu"]) && $_GET["id_pelaporan_tamu"])) {
        $response = [
            "status_code" => 400,
            "data" => null,
    ...
    $query = "
        SELECT t.id, a.id, da.id, kec.id, kab.id, p.id, n.id, n.nama, n.bendera, n.status_aktif, 
            ...
            plt.keterangan, plt.waktu, 
            plt.latitude, plt.longitude
    ...
    $stmt->bind_result($tamu_id, $akomodasi_id, $desa_adat_id, $kecamatan_id, 
    ...
    ";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ss", $tamu_id, $tamu_id);
