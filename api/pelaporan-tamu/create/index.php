<?php
    ...
    $stmt->bind_result($akomodasi_id, $desa_adat_id, $kecamatan_id, $kabupaten_id, $provinsi_id, 
        ...
        $akomodasi_cover, $akomodasi_description, $akomodasi_logo, $akomodasi_name, $akomodasi_location, 
        $akomodasi_active_status, $negara_tamu_id, $negara_tamu_name, $negara_tamu_flag, $negara_tamu_active_status, 
        $name, $email, $avatar, $phone, $date_of_birth, $identity_type, $identity_number, $gender, $active_status, $valid_status);
    $stmt->fetch();
    $stmt->close();
    ...
    if (count($radius_result_pecalang) > 0) {
        ...
        if (count($radius_result_desa_adat) == 0) {
            $response = [
                "status_code" => 400,
                "data" => null,
                "message" => "Nearby village not found"
            ];
            echo json_encode($response);
            exit();
        }
    ...
    $query = "INSERT INTO pelaporan_tamu VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("sssssssss", $id, $report_desa_adat_id, $id_jenis_pelaporan, 
        $title, $photo, $description, $now, $latitude, $longitude);
    ...
    while ($stmt->fetch()) {
        $query = "INSERT INTO notifikasi_tamu VALUES (NULL, ?, ?, ?, ?, '1', ?)";
        $stmt2 = $mysqli->prepare($query);
