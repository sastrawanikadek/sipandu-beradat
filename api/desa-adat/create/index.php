<?php
    ...
    $query = "SELECT COUNT(*) FROM desa_adat da WHERE da.nama = ? AND da.id_kecamatan = ?";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ss", $name, $id_kecamatan);
    ...
    $query = "INSERT INTO desa_adat VALUES (NULL, ?, ?, ?, ?, 1)";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ssss", $id_kecamatan, $name, $latitude, $longitude);
