<?php
    ...
    $query = "SELECT COUNT(*) FROM sirine_akomodasi sa WHERE sa.id_akomodasi = ? AND sa.kode = ?";
    $stmt = $mysqli->prepare($query);
    ...
    $query = "INSERT INTO sirine_akomodasi VALUES (NULL, ?, ?, ?, ?, 1)";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ssss", $id_akomodasi, $code, $photo, $location);
    ...
    $response = [
        ...
        "message" => "Data sirine akomodasi berhasil ditambahkan"
    ];
    echo json_encode($response);