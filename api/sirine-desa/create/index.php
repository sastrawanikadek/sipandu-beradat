<?php
    ...
    $query = "SELECT COUNT(*) FROM sirine_desa sd WHERE sd.id_desa = ? AND sd.kode = ?";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ss", $id_desa, $code);
    ...
    $query = "INSERT INTO sirine_desa VALUES (NULL, ?, ?, ?, ?, 1)";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("ssss", $id_desa, $code, $photo, $location);
    ...
    $response = [
        ...
        "message" => "Data sirine desa adat berhasil ditambahkan"
    ];
    echo json_encode($response);