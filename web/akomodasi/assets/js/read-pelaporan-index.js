...
const readAllPelaporan = async () => {
  ...
  const data = [...pelaporanDaruratsTamu, ...pelaporansTamu];

  setupFilterDataTable(
    "table-pelaporan-index",
    [7],
    data.map((obj, i) => [
      i + 1,
      obj.time,
      ...
      <a href="pages/pelaporan/detail-pelaporan.html?id=${obj.id}&report_type=${obj.report_type}&latitude=${obj.latitude}&longitude=${obj.longitude}" class="btn btn-inverse-primary btn-rounded btn-icon btn-action mr-2 btn-detail" title="Detail" >
