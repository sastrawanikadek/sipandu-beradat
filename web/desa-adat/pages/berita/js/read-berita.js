...
const readBerita = async () => {
  ...
  const req = await fetch(
    `https://sipanduberadat.com/api/berita/desa-adat/?id_desa_adat=${idDesa}`
  );
  ...
  if (status_code === 200) {
      ...
      <div class="col-xl-4 col-md-6 stretch-card grid-margin stretch-card">
        <div class="card">
            ...
            <div class="d-flex justify-content-between align-items-center">
              ...
              <span class="dropdown dropleft d-block">
                <span id="dropdownMenuButton1" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
                  <span><i class="mdi mdi-dots-horizontal-circle text-light" style="font-size: 1.3rem; cursor: pointer;"></i></span>
                </span>
          ...
          <div class="card-body">
            ...
            <p class="card-text content">${obj.content.substring(0, 150)}</p>
            <p class="card-text"><small class="text-muted">${full_date}</small></p>
          </div>
          <a class="text-primary p-4 mb-0 d-block h6 text-right" href='detail-berita.html?id_berita=${
            obj.id
          }'
          style="z-index:999;">Selengkapnya <i class="mdi mdi-chevron-right"></i></a>          
        </div>
      </div>`;
      $("#berita").append(row);
    });
    ...
    $("#berita").on("click", ".btn-edit", (e) => {
      ...
      const tes = CKEDITOR.instances["edit-content"].setData(content);
