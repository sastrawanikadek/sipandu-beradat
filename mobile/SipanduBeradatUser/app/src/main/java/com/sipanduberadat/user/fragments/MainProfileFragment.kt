...
import com.sipanduberadat.user.activities.VerifyCodeActivity
import com.sipanduberadat.user.services.FileDataPart
import com.sipanduberadat.user.services.apis.changeAvatarAPI
import com.sipanduberadat.user.services.apis.logoutAPI
...
class MainProfileFragment: Fragment() {
    ...
    private fun onSuccessChangeAvatar(response: Any?) {
        if (response != null) {
            viewModel.me.value!!.masyarakat.avatar = response as String
    ...
    override fun onCreateView(
        ...
        viewModel.me.observe(activity!!, {
            if (it != null) {
                val locationText = "Banjar ${it.masyarakat.banjar.name}, " +
                        "Desa Adat ${it.masyarakat.banjar.desa_adat.name}"
                Glide.with(view.context).load(it.masyarakat.avatar).centerCrop().into(view.avatar)
                if (it.masyarakat.valid_status || it.pecalang != null) {
                    view.chip_container.visibility = View.VISIBLE
                    if (it.masyarakat.valid_status) view.verified.visibility = View.VISIBLE
                    if (it.pecalang != null) {
                        view.pecalang.visibility = View.VISIBLE
                        view.pecalang.text = if (it.pecalang!!.prajuru_status) "Prajuru" else "Pecalang"
                    }
                }
