...
import com.sipanduberadat.user.activities.BlockedRoadActivity
import com.sipanduberadat.user.adapters.NewsAdapter
import com.sipanduberadat.user.adapters.NewsHeadlineCoverViewPagerAdapter
import com.sipanduberadat.user.adapters.NewsHeadlineTitleViewPagerAdapter
...
class MainNewsFragment: Fragment() {
    ...
    private fun onInitNews() {
        if (view != null && viewModel.reports.value != null && viewModel.guestReports.value != null
                && viewModel.news.value != null && viewModel.blockedRoads.value != null) {
            ...
            viewModel.blockedRoads.value!!.map { headline.add(BeritaWrapper(null, it, null,
                    null)) }
            viewModel.news.value!!.map {
                if (headline.size < headlineMaxSlide) headline.add(BeritaWrapper(it, null,
                        null, null))
                else news.add(BeritaWrapper(it, null, null, null))
            }
            viewModel.reports.value!!.filter { it.status >= 1 }.map {
                news.add(BeritaWrapper(null, null, it, null)) }
            viewModel.guestReports.value!!.filter { it.status >= 1 }.map {
                news.add(BeritaWrapper(null, null, null, it)) }
            headline.sortByDescending { if (it.blockedRoad != null) it.blockedRoad!!.start_time.time else
                it.news!!.time.time }
    ...
    override fun onCreateView(
        ...
        viewModel.me.observe(activity!!, {
            if (it != null) {
                view.btn_add.visibility = if (it.pecalang != null) View.VISIBLE else View.GONE
                view.btn_add.setOnClickListener {
                    val intent = Intent(view.context, BlockedRoadActivity::class.java)
                    startActivityForResult(intent, 1)
                }
                view.recycler_view.apply {
                    layoutManager = FlexboxLayoutManager(view.context, FlexDirection.ROW,
                            FlexWrap.WRAP)
                    adapter = NewsAdapter(view.context, news, viewModel.me.value!!)
                }
            }
        })
        ...
        view.swipe_refresh.setOnRefreshListener { onRefresh() }
