...
import com.sipanduberadat.user.activities.MainActivity
import com.sipanduberadat.user.services.apis.addLocationHistoryAPI
import com.sipanduberadat.user.services.apis.updateLocationPecalangAPI
...
class FirebaseNotificationService: FirebaseMessagingService() {
    ...
    override fun onMessageReceived(p0: RemoteMessage) {
        ...
        when (data["notification_type"]) {
            ...
            "request-location-pecalang" -> {
                requestLocation(this) {
                    val requestParams = HashMap<String, String>()
                    requestParams["latitude"] = "${it.latitude}"
                    requestParams["longitude"] = "${it.longitude}"

                    updateLocationPecalangAPI(View(this), this, requestParams, HashMap(),
                            {}, {}, showMessage = false)
                }
            }
            "family-request" -> {
            ...
            "report" -> {
                val isKrama = JSONObject(data["notification_data"]!!).has("type")
                val withPhoto = data["notification_photo"] != null
                val soundUri: Uri = if (!isKrama)
                    Uri.parse("android.resource://${packageName}/${R.raw.complaint}") else
                ...
                Glide.with(this).asBitmap().load(if (withPhoto) data["notification_photo"] else R.drawable.logo)
                    ...
                    override fun onResourceReady(resource: Bitmap, transition: Transition<in Bitmap>?) {
                        val notification = NotificationCompat.Builder(this@FirebaseNotificationService,
                            ...
                            setSound(soundUri)
                            setContentIntent(pendingIntent)
                            setCategory(if (!isKrama) NotificationCompat.CATEGORY_CALL else
                                NotificationCompat.CATEGORY_SOCIAL)
                            if (!isKrama) setOngoing(true)
                            setAutoCancel(true)
                            setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
                            color = ContextCompat.getColor(this@FirebaseNotificationService,
                                R.color.red_700)
                        ...
                        if (!isKrama) notification.flags = notification.flags or
            ...
            "emergency-report" -> {
                ...
                val withPhoto = data["notification_photo"] != null
                val soundUri: Uri = if (!isKrama)
                    Uri.parse("android.resource://${packageName}/${R.raw.emergency}") else
                    RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION)
                val intent = Intent(this, MainActivity::class.java)
