...
import com.sipanduberadat.guest.services.apis.refreshTokenAPI
...
fun getRelativeDateTimeString(millis: Long): String {
    ...
    if (minute < 1) {
        ...
        if (hour < 1) {
            result = if (minute > 1) "$minute minutes ago" else "$minute minute ago"
        } else {
            day = (hour / 24).toFloat().roundToLong()

            if (day < 1) {
                result = if (hour > 1) "$hour hours ago" else "$hour hour ago"
            } else {
                month = (day / 30).toFloat().roundToLong()

                if (month < 1) {
                    result = if (day > 1) "$day days ago" else "$day day ago"
                } else {
                    year = (month / 12).toFloat().roundToLong()
                    val monthText = if (month > 1) "$month months ago" else "$month month ago"
                    val yearText = if (year > 1) "$year years ago" else "$year year ago"

                    result = if (year < 1) monthText else yearText
                }
            }
        }
    }
...
fun getTimeCountdown(startTime: Long, endTime: Long, timeFormat: Boolean = false): String {
    ...
    val daysName = if (days > 1) "days" else "day"
    val hoursName = if (hours > 1) "hours" else "hour"
    val minutesName = if (minutes > 1) "minutes" else "minute"
    val secondsName = if (seconds > 1) "seconds" else "second"

    return if (timeFormat) String.format("%02d:%02d:%02d", hours, minutes, seconds) else
        String.format("%02d $daysName, %02d $hoursName, %02d $minutesName, %02d $secondsName",
...
fun choosePhoto(): Intent {
    ...
    val intentChooser = Intent.createChooser(galleryIntent, "Choose photo source")
