...
fun findAllRequestKerabatTamuAPI(contextView: View, context: Context, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(context, Request.Method.POST,
                "${Constants.BASE_URL}/kerabat-tamu/?family_status=0", requestParams, fileRequestParams, {
            val dataString = getResponseData(
                    it, contextView, context, requestParams,
                    fileRequestParams, callback, errorCallback, ::findAllRequestKerabatTamuAPI, showMessage
            )
