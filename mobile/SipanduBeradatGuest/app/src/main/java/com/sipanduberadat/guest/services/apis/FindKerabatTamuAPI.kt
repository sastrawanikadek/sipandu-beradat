...
fun findKerabatTamuAPI(contextView: View, context: Context, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(context, Request.Method.POST,
                "${Constants.BASE_URL}/kerabat-tamu/find/", requestParams,
                fileRequestParams, {
            val dataString = getResponseData(it, contextView, context, requestParams,
                    fileRequestParams, callback, errorCallback, ::findKerabatTamuAPI, showMessage)

            if (dataString != null) {
                val gson = GsonBuilder().setDateFormat("yyyy-MM-dd' 'HH:mm:ss").create()
                val kerabat = gson.fromJson(dataString, KerabatTamu::class.java)
                callback(kerabat)
            }
        }, { errorCallback() }).start()
        return
