...
fun meAPI(contextView: View, context: Context, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(context, Request.Method.POST,
            "${Constants.BASE_URL}/petugas/me/", requestParams,
            fileRequestParams, {
                ...
                if (dataString != null) {
                    val me = Gson().fromJson(dataString, Petugas::class.java)
                    callback(me)
                }
            }, { errorCallback() }).start()
        return
