...
import com.sipanduberadat.petugas.models.PenutupanJalan
...
fun createPenutupanJalanAPI(contextView: View, context: Context, requestParams: HashMap<String, String>,
    ...
    if (accessToken != null) {
        ...
        VolleyMultipartRequest(context, Request.Method.POST,
                "${Constants.BASE_URL}/penutupan-jalan/create/", requestParams,
                fileRequestParams, {
            val dataString = getResponseData(it, contextView, context, requestParams,
                    fileRequestParams, callback, errorCallback, ::createPenutupanJalanAPI, showMessage)

            if (dataString != null) {
                val gson = GsonBuilder().setDateFormat("yyyy-MM-dd' 'HH:mm:ss").create()
                val report = gson.fromJson(dataString, PenutupanJalan::class.java)
                callback(report)
            }
        }, { errorCallback() }).start()
        return
